...
import useMutationCart, { deleteItem, clearCart, updateCount } from '../hooks/useMutationCart';
...
export default function Cart() {
  ...
  let { data, isError, error, isLoadind } = useQueryCart(getCarts)
  console.log(data);


  let { mutate, data: deletedata, isLoading } = useMutationCart(deleteItem)
  console.log('deletedata', deletedata);
  let { mutate: mutateClear, data: cleardata, ispending: ispendingClear } = useMutationCart(clearCart)
  let { mutate: mutateupdate } = useMutationCart(updateCount)
  ...
  return (
    ...
    <div className="w-3/4 mx-auto  my-5 relative overflow-x-auto  sm:rounded-lg">
      <h1 className='font-bold my-10'>Number  of Cart Items: {data?.data?.numOfCartItems}</h1>
      <h1 className='font-bold my-10'>Number  of Cart Items: {data?.data?.totalCartPrice}EGP</h1>
        ...
        <tbody>
          {data?.data?.data?.products.map((prod =>
            <tr key={prod?.product?._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
              ...
              <td className="px-6 py-4">
                <a onClick={() => { mutate(prod?.product?._id) }} className=" cursor-pointer font-medium text-red-600 dark:text-red-500 hover:underline">Remove<i className='fa-solid fa-trash'></i></a>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <button className='bg-green-500 p-4 my-10 float-right cursor-pointer' onClick={mutateClear}>Clear Cart</button>
      <br />
      <button onClick={() => { setOpen(!isOpen) }} className='bg-green-500 p-4 my-10  cursor-pointer' >Pay Online</button>
